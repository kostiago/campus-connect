...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderEvents(categoria) {
    ...
    eventsGrid.innerHTML = eventosFiltrados.map((evento) => `
      <article class="event-item-card">
        ...
        <div class="event-item-body">
          ...
          <div class="event-item-footer">
            <button class="btn-details">Ver detalhes ${arrowIconSvg}</button>
          </div>
        </div>
      </article>
    `).join("");
 
    eventsEmpty.style.display = eventosFiltrados.length === 0 ? "flex" : "none";
    eventsGrid.style.display = eventosFiltrados.length === 0 ? "none" : "grid";
  }
